...
for ds_name_7 in data_list_7:
    ...
    # ship_track_mask = np.logical_and(refl_var_ch06 > 0.28, refl_var_ch02 > 0.3)
    ###########################################################################
    ...
    # size_mask = np.logical_and(np.logical_and(var_aer > 0, var_aer < size_func), np.logical_and(np.logical_not(np.isnan(var_aer)), np.logical_and(var_aer < 20, var_depth < 30)))
    # size_mask = np.logical_and(np.logical_and(var_aer > size_func-0.3, var_aer < size_func + 0.3), np.logical_and(np.logical_not(np.isnan(var_aer)), np.logical_and(var_aer < 20, var_depth < 30)))
    # size_mask = np.logical_and(np.logical_and(var_aer > 0, var_aer < size_func), np.logical_not(np.isnan(var_aer))) # August 08
    # size_mask = np.logical_and(np.logical_and(var_aer > size_func - 3, var_aer < size_func), np.logical_not(np.isnan(var_aer)))
    ...
    # norm_BTD = calc_BTD.main_func_norm(var_ch14, var_ch07, 14, 7)
    # min_aer = np.nanmin(norm_BTD)
    # if min_aer < 0:
    #     norm_BTD = norm_BTD + np.abs(min_aer)
    # max_aer = np.nanmax(norm_BTD)
    # norm_BTD = norm_BTD/max_aer
    # norm_BTD = cv2.cvtColor(norm_BTD*255, cv2.COLOR_GRAY2BGR)
    # filename_aer = "NEW_BTD_" + str(i) + ".png"
    # file_path_aer = os.path.join(OUT_DIR, filename_aer)
    # cv2.imwrite(file_path_aer, norm_BTD)
    ...
    # min_aer = np.nanmin(refl_var_ch02)
    # if min_aer < 0:
    #     refl_var_ch02 = refl_var_ch02 + np.abs(min_aer)
    # max_aer = np.nanmax(refl_var_ch02)
    # refl_var_ch02 = refl_var_ch02/max_aer
    # refl_var_ch02 = cv2.cvtColor(refl_var_ch02*255, cv2.COLOR_GRAY2BGR)
    # filename_aer = "ch2_" + str(i) + ".png"
    # file_path_aer = os.path.join(OUT_DIR, filename_aer)
    # cv2.imwrite(file_path_aer, refl_var_ch02)

    # min_aer = np.nanmin(refl_var_ch06)
    # if min_aer < 0:
    #     refl_var_ch06 = refl_var_ch06 + np.abs(min_aer)
    # max_aer = np.nanmax(refl_var_ch06)
    # refl_var_ch06 = refl_var_ch06/max_aer
    # refl_var_ch06 = cv2.cvtColor(refl_var_ch06*255, cv2.COLOR_GRAY2BGR)
    # filename_aer = "ch6_" + str(i) + ".png"
    # file_path_aer = os.path.join(OUT_DIR, filename_aer)
    # cv2.imwrite(file_path_aer, refl_var_ch06)
    ###############################
